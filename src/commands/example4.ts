...
async function main() {
  try {
    ...
    const interfacePath = path.join(process.env.SOURCE_DIR, 'example4.interface.ts');
    ...
    const baseName = 'Example4Payload';
    ...
    const buildPath = path.join(process.env.OUT_DIR, 'example4.schema.json');
